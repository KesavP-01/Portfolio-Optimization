...
def optimization(data, no_of_portfolios):
    ...
    n = no_of_portfolios
    ind_returns = data.resample('M').last().pct_change().mean()
...
def Min_vol(data):
    return data.iloc[data['Volatility'].idxmin()]

def optimal_risk(data, risk_free_rate):
    rf = risk_free_rate
    return data.iloc[((data['Returns']- rf)/data['Volatility']).idxmax()]
...
Minimum_volatility = Min_vol(Optimized_portfolios)
Optimal_risk = optimal_risk(Optimized_portfolios, 0.01)
